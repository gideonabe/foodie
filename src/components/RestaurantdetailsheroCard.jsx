...
export const RestaurantdetailsheroCard = ({time, shorttext, maintext, minimumOrder, deliveryTime, imageUrl, offertext, rate, reviews, rating}) => {
  ...
  return (
    <>
      ...
      <div className="relative bg-[url('/assets/burgerbg.png')] bg-cover bg-center bg-no-repeat w-full rounded-xl py-4 px-4 md:px-2 md:py-2">
        ...
        <div className="absolute -bottom-5 left-0 z-20">
          <span className="flex justify-center items-center gap-1 bg-orange-400 text-white px-8 py-2 text-sm rounded-tr-lg rounded-br-lg font-medium shadow-md">
            <FaClock size={15}/> {time}
          </span>
        </div>

        {/* Content */}
        <div className="relative z-10 flex flex-col md:flex-row items-center h-full gap-2 md:gap-10 p-6 md:p-8 text-white">

          {/* Left side content */}
          <div className="flex flex-1 flex-col gap-3 max-w-[100%] md:max-w-[70%]">
            <p className="text-sm md:text-base text-gray-300">{shorttext}</p>
            <h1 className="text-2xl md:text-[40px] font-bold leading-tight">{maintext}</h1>
            ...
            <div className="flex gap-3 mt-2 flex-wrap">
              <div className="flex justify-center items-center gap-2 px-4 py-2 rounded-full text-sm md:text-base border border-white/20">
                <span className='flex justify-center items-center gap-2'><TbChecklist size={25}/>{minimumOrder}</span> 
              </div>
              <div className="flex items-center gap-2 px-4 py-2 rounded-full text-sm md:text-base border border-white/20">
                <span className='flex justify-center items-center gap-2'><LuBike size={25}/>{deliveryTime}</span> 
              </div>
            </div>
          </div>

          {/* Right side image and rating */}
          <div className="relative flex-1 justify-center w-full h-full mt-6 md:mt-0">
            ...
            <div className="absolute -bottom-2 -left-8  text-black rounded-xl shadow-md bg-white py-1  px-1 w-18 md:w-22 md:py-3 md:px-4 text-center flex flex-col justify-center items-center">
              <p className="text-2xl md:text-4xl font-semibold">{rate}</p>
              <p className="text-yellow-400 text-sm leading-none">{rating}</p>
      ...
      <div className=" max-w-[95%] md:max-w-[90%] mx-auto flex flex-col md:flex-row justify-between items-center mt-10 md:mt-8 px-4 md:px-0 gap-4">
        <h2 className="text-base md:text-xl font-bold">{offertext}</h2>
